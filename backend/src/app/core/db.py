...
def seed_employees(conn: sqlite3.Connection, rows: Iterable[Mapping]) -> None:
    ...
    cur.executemany(
        """
        INSERT OR REPLACE INTO employees (id, name, role, department_id, level, points_current, hire_date, skills_map, courses_enrolled_map, goals_set)
        VALUES (:id, :name, :role, :department_id, :level, :points_current, :hire_date, :skills_map, :courses_enrolled_map, :goals_set)
        """,
        list(rows),
    )
    conn.commit()
